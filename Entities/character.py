...
class Character:
    """Класс Character используется для создания объектов с параметрами, соответствтующими уровню
        ----------
        Attributes
            health - int, величина, отвечающая за здоровье персонажа
            armor - int, величина, отвечающая за броню
            damage - int, величина, отвечающая за наносимый противнику урон
            hero - bool, флаг, отличающий основного персонажа и противников
        ----------
        Methods
            __init__(lvl, hero)
                метод-конструктор, создающий персонажа
            __bool__()
                возвращает True, если персонаж жив (health > 0)"""
    ...
    def __bool__(self):
        """Перегрузка булевой операции, возвращает True, если персонаж жив (health > 0)"""
