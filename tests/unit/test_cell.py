"""Тест-кейс проверяет метод класса Cell"""
...
class TestCell(unittest.TestCase):
    def test_get_entity_type(self):
        """Тест проверяет работу метода get_entity_type, конкретно, что при изменении здания, entity_type меняется
        корректно"""
        cell1 = Cell(Building.ALTAR, FowMode.REVEALED)
        cell1.update_entity_type()
